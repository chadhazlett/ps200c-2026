...
setwd("~/Dropbox/chad_transfer/UCLA/PS200D/2016/slides/04SOO")
...
signif(btest,3)
signif(btest[,c("mean.Tr","mean.Co","T pval","KS pval")],2)
...
#Display output, rounding to two digits
signif(btest_after[,c("mean.Tr","mean.Co","T pval","KS pval")],2)

#See balance before and after next to each other
balancecompare=cbind(signif(btest[,c("mean.Tr","mean.Co","T pval","KS pval")],2),#
                     signif(btest_after[,c("mean.Tr","mean.Co","T pval","KS pval")],2))
...
#pdf("IPWdensity.pdf", height=4, width=6)
...
#pdf("pscores_post.pdf", height=4, width=6)
...
#We'll adjust the last one by hand...don't want all the coefficients
stargazer(lm.out.naive, lm.out.ipw, lm.out.ipw)


### Let's plot balance again with a few more results:
#First get balance from IPW weighting
