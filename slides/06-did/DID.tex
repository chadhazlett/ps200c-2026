...
\scriptsize
\textbf{Setup.} DGP A: half the units treated at one time, others never-treated. \textit{Parallel trends holds.} Post-treatment effect is positive, builds slowly.
\medskip
...
Use it to (a) visualize effect dynamics ($\beta_k$, $k\ge 0$); (b) check leads ($k<0$) $\approx 0$---a visual (pre)-PT/placebo diagnostic.
\begin{center}
...
\item All ``modern'' methods (SA, CS, BJS / \texttt{fect}) give (correct) event-study plots.
\end{itemize}
\end{frame}
...
\small
\begin{itemize}
\item<1-> If time-varying influences might lead to treatment and changes in ``growth'', i.e. $Y_t(0)-Y_{t-1}(0)$, then parallel trends is violated. 
\medskip
\item<2-> Modern DID methods don't touch this. 
\medskip
\item<3-> Parallel trends (in the relevant period) is never testable. Don't be fooled by pre-trend tests. \medskip
\end{itemize} 
\bigskip
\end{frame}
