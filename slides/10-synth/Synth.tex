...
\begin{frame}{EIV as $T_{pre}$ grows}
\scriptsize
Changing momentarily to $N=50$, check imbalance on $U$ (averaging the 3 dims) as $T_{\text{pre}}$ grows. Bias in $Y(0)$ would be proportional to this. (Dashed: best-fit $c/\sqrt{T_{\text{pre}}}$).
\begin{center}
\includegraphics[width=.78\textwidth]{eiv_u_imbalance.pdf}
\end{center}
%\onslide<2-> Empirical curve tracks $c/\sqrt{T_{\text{pre}}}$ closely ($R^2 \approx 0.997$, $\hat c \approx 2.56$)
\end{frame}
