...
\begin{frame}{The stakes are real: Hormone Replacement Therapy}
\small
\onslide<1-> For decades, \textbf{40+ observational studies} consistently showed that postmenopausal women on hormone replacement therapy (HRT) had $\sim$30\% lower rates of heart disease.\\
\bigskip
\onslide<2-> Panels of experts endorsed HRT. Millions of women were prescribed it.\\
\bigskip
\onslide<3-> But these were observational comparisons: women who \textit{chose} HRT tended to be wealthier, healthier, and more engaged with the medical system---classic \textbf{healthy user bias}.\\
\bigskip
\onslide<4-> ...what do we really learn from such observational evidence?
\end{frame}

\begin{frame}{HRT: The danger of observational claims}
\onslide<1-> The Women's Health Initiative (2002) randomized $\sim$16,000 women to HRT or placebo. It found a $\sim$29\% \textit{increase} in heart attacks and elevated cancer risk.\\
\medskip
\onslide<2-> The trial was \textbf{halted early}. 
\begin{itemize}
\item Physicians reversed course overnight
\item  Millions of women stopped HRT
\item Even now people take guidance from that trial.
\end{itemize}
\end{frame}


\begin{frame}{Here we go again}
\onslide<1-> That interpretation of the trial result may have been hasty.\\
\bigskip
\onslide<2-> The WHI enrolled women who were on average \textbf{63 years old}---10--20 years past menopause.\\

\begin{itemize}
\item Later reanalysis showed that for women near menopause, HRT likely \textit{is} protective. 
\item The ``timing hypothesis'' is now widely accepted, and guidelines have largely come back around.\\
\end{itemize}
\bigskip
\onslide<4-> \textbf{Lesson:} Even a large RCT doesn't necessarily tell you what you want.\\

\begin{itemize}
 \item<5-> Design details (the treatment formulation, who is enrolled), stopping rules, interpretation can invite errors
 \item<6-> An experiment answers a causal question, but \textit{which} causal question depends on details you cannot ignore.
 \end{itemize}
\end{frame}

\begin{frame}{More examples to keep in mind}
\small
\begin{itemize}
\makebiggaps
\item<1-> \textbf{Scared Straight programs:} Observational studies suggested exposing at-risk youth to prison life reduced delinquency. Multiple RCTs later showed it made outcomes \textit{worse}. Programs were widely implemented before the evidence caught up.
\medskip
\item<2-> \textbf{Microfinance:} Celebrated as a path out of poverty based on observational comparisons of borrowers vs.\ non-borrowers. Six major RCTs found much more modest effects---helpful in some ways, but not transformative.
\medskip
\item<3-> \textbf{Smoking and lung cancer:} We will never have an RCT. Yet the causal conclusion is among the most trusted in all of science. Why? Not because of the volume of data, but because researchers could formalize \textit{what it would take} for the result to be wrong---and the required confounding was implausible. (We will return to this when we cover sensitivity analysis.)
\end{itemize}
\end{frame}
...
\frametitle{Modes of Statistical Inference}
  ...
  \item[1.] Descriptive Inference: summarizing and exploring data
    ...
    \item Note: the statistical inference you are used to---hypothesis tests, $p$-values, confidence intervals---is \textit{descriptive} inference. It furthers the description of how variables look or move together. It does not, by itself, get you any closer to causal.
    \end{itemize}
    \medskip \pause
...
\begin{itemize}
\item Yes.
\item Why? Because the assumptions required to say things about unobservable counterfactuals are understandably onerous.
\end{itemize}
\medskip
\normalsize
...
\end{itemize}
\small
\onslide<3-> In fact, I hope the course helps to convince you that technical sophistication of methods often does not solve core problems...it's not about the latest estimator, machine learning, or more complicated standard errors.
\end{frame}
...
\end{itemize}
\onslide<3-> In other words, ``looking at the data'' is not a sufficient strategy for learning about the world... \\
\bigskip
\onslide<4-> So what \textit{does} it take? That's what this course is about.
\end{frame}


\begin{frame}{Identification: Assumptions + Data $\rightarrow$ Causal Conclusions}
\footnotesize
\onslide<1-> Causal inference is about that ``further assumption'' bit:
\begin{itemize}
\item<2-> What assumptions let us connect the causal thing we want to know to something in the data?
\item<3-> Can we \textcolor{blue}{identify} our quantity of interest with something we can estimate?
\item<4-> We speak of \textcolor{blue}{identification assumptions} or \textcolor{blue}{identification strategies} that provide such bridges.
\end{itemize}
\medskip
\onslide<5-> Key implications:
\begin{itemize}
\item<6-> Most identifying assumptions cannot be proven correct using data. It's not about ``knowing the right test'' to run.
\item<7-> We will always be appealing to information, assumptions, or context from \textit{outside} the data.
\item<8-> Credibility does NOT derive from ``what method you used.'' There are not ``causal methods''! You can't say ``it's causal because I did xyz.''
\item<9-> This is one of the hardest things for many students to absorb, in part because we are surrounded by mistakes and false advertising.
\end{itemize}
\end{frame}

\begin{frame}{You cannot escape causal reasoning}
\footnotesize
\onslide<1-> You might think causal inference is only for people making ``clearly causal claims,'' and only when they can't run an experiment. Both are wrong.\\
\medskip
\begin{itemize}
\item<2-> Many ``comparisons'' are only interesting because the audience will read them as suggestive of a causal relationship. If you compare outcomes across groups, you are implicitly in causal territory whether you say so or not.
\medskip
\item<3-> Even genuinely descriptive tasks invoke causal reasoning. Handling \textbf{missingness} requires assumptions about \textit{why} data are missing---a causal question. Choosing what to \textbf{condition on} requires knowing the causal structure, or you risk conditioning on a collider and creating bias that wasn't there before.
\medskip
\item<4-> Randomized experiments don't make causal reasoning unnecessary---they make it \textit{more} necessary. What \textbf{mechanism} produced the total effect? How do you handle \textbf{attrition} (which is post-treatment)? How do you choose covariates for \textbf{precision} without introducing bias? These all require understanding causal structure.
\medskip
\item<5-> Even \textbf{generalization}---asking whether a result applies beyond the study sample---is a causal question about what would happen under different conditions.
\end{itemize}
\end{frame}
...
\item<4-> \textbf{How can you conduct top quality work that seeks to make causal claims in tricky situations, while accurately characterizing what can and cannot be claimed?}
\end{itemize}
\end{frame}
...
\medskip
\item<4-> \textbf{My office hours:} Wednesday 3:30--4:45 PM and by appointment, Bunche 3264.
\medskip
